...
const Nav = () => {
    ...
    return (
        <div className='sticky shadow-md w-full fixed top-0 left-0 relative z-10'>
            ...
            <div onClick={()=>setOpen(!open)} className='text-3xl text-gray-100 absolute right-8 top-6 cursor-pointer md:hidden
            '>
            <ion-icon name={open ? 'close':'menu'}></ion-icon>   
            </div>

            <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static
            bg-gray-900 md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500
            ease-in ${open ? 'top-20 opacity-100':'top-[-490px]'} md:opacity-100 opacity-0`}>
                {
                    Links.map((Link)=>(
                        <li key={Link.name} className='md:ml-8 text-xl md:my-0 my-7'>
                            <a href={Link.link} className='text-gray-100 hover:text-yellow-500'>{Link.name}</a>
                        </li>
                    ))    
                }
            </ul>   
